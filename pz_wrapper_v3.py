...
class SupervisorWrapper(gym.Env):
    ...
    def __init__(self, pz_env: AECEnv, aggregation_method='neural'):
        super().__init__()
        self.env = pz_env
        self.env.reset()
        ...
        if aggregation_method == 'sum':
          ...
          self.agg_func = self.__reward_weighted_mean
        elif aggregation_method == 'neural':
          self.agg_func = self.__reward_neural

    
    def __reward_neural(self, rewards):
    ...
    def __reward_weighted_mean(self, rewards, epsilon=1e-6):
      w = 1 / (np.array(rewards) + epsilon)
      w /= w.sum()
    ...
    def step(self, action):
        ...
        self.joint_action[self.env.agents[self.current_agent_idx]] = action
        ...
        if len(self.joint_action) < self.num_agents:
            ...
            return self._get_observations(action), self.agg_func([0] * len(self.env.possible_agents)), False, False, {}

        rewards, terminations, truncations = {agent:0 for agent in self.env.possible_agents}, [], []
        for agent, action in self.joint_action.items():
    ...
    def _add_action_info(self, action=None):
      ...
      if action is None:
        ...
        aux = []
        for x in self.joint_action.values():
